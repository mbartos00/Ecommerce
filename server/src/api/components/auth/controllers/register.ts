...
export function register({ prisma }: Dependecies) {
  return async (req: Request<{}, {}, RegisterSchema>, res: Response) => {
    ...
    const avatarPath = req.file
      ? `${process.env.NODE_ENV === 'production' ? 'https' : 'http'}://${process.env.DOMAIN}:${process.env.PORT}/uploads/${sanitizeFilename(req.file.filename)}`
      : `${process.env.NODE_ENV === 'production' ? 'https' : 'http'}://${process.env.DOMAIN}:${process.env.PORT}/uploads/defaultAvatar.png`;
